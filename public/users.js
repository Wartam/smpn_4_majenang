...
userForm.addEventListener('submit', async (event) => { event.preventDefault(); userError.textContent = ''; const values = Object.fromEntries(new FormData(userForm)); if (!values.password) delete values.password; const url = editingUser ? `/api/admin/users/${editingUser}` : '/api/admin/users'; try { const response = await fetch(url, { method: editingUser ? 'PUT' : 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(values) }); const result = await response.json(); if (!response.ok) { userError.textContent = result.error || 'Gagal menyimpan pengguna'; return } closeUser(); await loadUsers(); toast(editingUser ? 'Pengguna berhasil diperbarui' : 'Pengguna berhasil ditambahkan') } catch { userError.textContent = 'Server tidak dapat dihubungi. Coba lagi.' } })
